...
foreach ($users as $u) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        $uStmt = $conn->prepare("UPDATE `USER` SET password=?, full_name=?, role=? WHERE user_id=?");
        $uStmt->bind_param('sssi', $hashed, $full_name, $role, $user_id);
        $uStmt->execute();
        $uStmt->close();
        ...
        $stmt->close();
        // insert
        $iStmt = $conn->prepare("INSERT INTO `USER` (username, password, full_name, role) VALUES (?, ?, ?, ?)");
        $iStmt->bind_param('ssss', $username, $hashed, $full_name, $role);
        $iStmt->execute();
        $iStmt->close();
